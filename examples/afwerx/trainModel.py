...
def main():
    ...
    parser.add_option(
        ...
        dest="inFeatMatFile",
        default=None,
        help="The input file containing the feature matrix.",
    )
    parser.add_option(
        "-m",
        "--modelType",
        ...
        dest="modelType",
        default=None,
        help='Keyword specifying the task being solved by this model:either "DifficultyLevel" or "PerfErr".',
    )
    parser.add_option(
        "-r",
        "--trainOutcomesFile",
        action="store",
        dest="trainOutcomesFile",
        default=None,
        help="Input file of training set outcomes.",
    )
    parser.add_option(
        "-v",
        "--valOutcomesFile",
        action="store",
        dest="valOutcomesFile",
        default=None,
        help="Input file of validation set outcomes.",
    )
    parser.add_option(
        "-t",
        "--testPredOutputFile",
        action="store",
        dest="testPredOutputFile",
        default=None,
        help="If specified, will run trained model on test subjects and save the predictions to this file path.",
    )
    parser.add_option(
        "-o",
        "--outDir",
        ...
        dest="outDir",
        default=None,
        help="Directory to save output validation ROC curves and other results.",
    )
    ...
    trainMatDF = featMatDF.merge(
        trainOutcomesDF, how="inner", on=["Subject", "Session", "Run"]
    )
    valMatDF = featMatDF.merge(
        valOutcomesDF, how="inner", on=["Subject", "Session", "Run"]
    )
    ...
    if options.modelType == "DifficultyLevel":
        ...
        inclDiffLevels = [1, 4]
        # The task is only to distinguish between difficulty levels 1 and 4.
        numPCs = 0
        [clf, pcaSetup_diffLevels] = trainDiffPredictionModel(
            trainMatDF, inclDiffLevels, featCols, numPCs
        )
        validateDiffPredictionModel(
            valMatDF,
            pcaSetup_diffLevels,
            inclDiffLevels,
            featCols,
        ...
        numPCs = 2
        [reg, pcaSetup_perfErr] = trainPerfErrPredictionModel(
            trainMatDF, featCols, numPCs
        )
        validatePerfErrPredictionModel(
            valMatDF, pcaSetup_perfErr, featCols, reg, options.outDir
        )
...
def formTestMat(featMatDF, trainOutcomesDF, valOutcomesDF):
    ...
    testFeatMatDF1 = featMatDF[
        ~featMatDF.Subject.str.contains("|".join(utrainSubjs))
    ]
    testFeatMatDF = testFeatMatDF1[
        ~testFeatMatDF1.Subject.str.contains("|".join(uvalSubjs))
    ]
...
def validatePerfErrPredictionModel(valMatDF, pcaSetup, featCols, mdl, outDir):
    ...
    plt.title(
        "Total Flight Error: Predicted vs. Actual\n MSE = "
        + str(mse)
        + "\nPearson Correlation: "
        + str(pearsonCorr)
    ...
    plt.savefig(
        outDir + os.path.sep + "valPerfErr_pred2actual.png", format="png"
    )
...
def trainDiffPredictionModel(trainMatDF, inclDiffLevels, featCols, numPCs):
    ...
    subTrainMatDF = trainMatDF.loc[
        trainMatDF["Difficulty"].isin(inclDiffLevels)
    ]
...
def validateDiffPredictionModel(
    valMatDF, pcaSetup, inclDiffLevels, featCols, clf, outDir
):
