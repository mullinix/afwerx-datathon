...
def computeDispersion(gazeX, gazeY, gazeZ):
    ...
    dispersion = np.sqrt(
        np.mean(offsetsXsq) + np.mean(offsetsYsq) + np.mean(offsetsZsq)
    )
...
def getSaccadeFeats(dfOcuEvt, dfRawEye, eoIdxs):
    ...
    sacFeats = computeSacMetrics(
        sacEO, timeEO, gazeX[eoIdxs], gazeY[eoIdxs], gazeZ[eoIdxs]
    )
...
def computeSacMetrics(sacEO, timeEO, gazeEOx, gazeEOy, gazeEOz):
    ...
    for sn in uniqueSacNums:
        ...
        curSacAmpl = np.sqrt(
            np.square(curSacGazeX[-1] - curSacGazeX[0])
            + np.square(curSacGazeY[-1] - curSacGazeY[0])
            + np.square(curSacGazeZ[-1] - curSacGazeZ[0])
        )

        curSacVels = np.sqrt(
            np.square(xvel) + np.square(yvel) + np.square(zvel)
        )
    ...
    df = y.shape[0] - 2
    # dof = N - k - 1, where N = # input data points and k = # variables = 1.
    ...
    T = fsolve(tstat, 1)
    # Calculate t-statistic for p = alpha and v = degrees of freedom
    T = abs(T)
    # Critical +ve value from t-distribution
    tsA = T * -1
    tsB = T
    # Create vector of t-statistic values
...
def t_cdf(t, v):
    ...
    IBx = v / (np.square(t) + v)
    # x for IxZW NOTE: function of t-statistic
    IBZ = v / 2
    # Z for IxZW
    IBW = 0.5
    # W for IxZW
    Ixzw = sc.betainc(IBx, IBZ, IBW)
