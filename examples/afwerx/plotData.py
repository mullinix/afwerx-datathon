...
def loadTimeSeries(dataRoot, subject, sessLabel, expType, modality, runLabel):
    ...
    print("Completed. Returning data frame.")
    return dfEDA
...
def plotTimeSeries(df, subject, sess, run, dataColumn, startIdx, endIdx):
    ...
    for c in dataColumn:
        plt.plot(df["time_s"][startIdx:endIdx], df[c][startIdx:endIdx], label=c)
    plt.xlabel("Time")
    plt.title(
        "Data excerpt for \nSubject: "
        + subject
...
def getTargetRows(featDF, perfDF, diffLevel):
    ...
    featRun = ["run-" + str(item).zfill(3) for item in perfRun]
...
def alignFeatPerfRows(featDF, perfDF):
    ...
    newPerfDF["Subject"] = [
        "sub-cp" + str(item).zfill(3) for item in perfDF["Subject"]
    ]
    newPerfDF["Session"] = ["ses-" + str(item) for item in perfDF["Date"]]
    newPerfDF["Run"] = ["run-" + str(item).zfill(3) for item in perfDF["Run"]]
...
def vizAggFeat_bySubj(featDF, perfDF, diffLevel, aggFeatColumn):
    ...
    plt.title(
        "Mean Values by Subject across Runs for "
        + aggFeatColumn
        + "\nDifficulty Level = "
        + str(diffLevel)
    )
...
def vizAggFeat_perfErr(featDF, perfDF, aggFeatColumn):
    ...
    cumErr = (
        joinDF["GlideSlope_Error"]
        + joinDF["Localizer_Error"]
        + joinDF["Airspeed_Error"]
    )
    ...
    plt.title(
        "Performance Error as a Function of "
        + aggFeatColumn
